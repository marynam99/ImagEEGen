...
def n_way_top_k_acc(pred, class_id, n_way, num_trials=40, top_k=1):
    ...
    for t in range(num_trials):
        ...
        acc = accuracy(pred_picked.unsqueeze(0), torch.tensor([0], device=pred.device), task="multiclass", num_classes=40,
                    top_k=top_k)
...
def get_n_way_top_k_acc(pred_imgs, ground_truth, n_way, num_trials, top_k, device, return_std=False):
    ...
    for pred, gt in zip(pred_imgs, ground_truth):
        ...
        pred = preprocess(Image.fromarray(pred.astype(np.uint8))).unsqueeze(0)
        gt = preprocess(Image.fromarray(gt.astype(np.uint8))).unsqueeze(0)
