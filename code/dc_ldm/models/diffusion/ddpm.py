...
class SDEncoder(pl.LightningModule):
...
class DDPM(pl.LightningModule):
    ...
    def training_step(self, batch, batch_idx):
        ...
        print(f'original_elbo_weight: {self.original_elbo_weight}')
    ...
    def full_validation(self, batch, state=None):
        ...
        grid, all_samples, state = self.generate(batch, ddim_steps=self.ddim_steps, num_samples=1, limit=None, state=state)
    ...
    def configure_optimizers(self):  ## DDPM
...
class LatentDiffusion(DDPM):
    ...
    def configure_optimizers(self): ## latent diffusion
        lr = self.learning_rate
        if self.train_cond_stage_only:
            ...
            # self.params = list(self.cond_stage_model.parameters()) 
        ...
        # opt = torch.optim.AdamW(self.cond_stage_model.parameters(), lr=lr)
        opt = torch.optim.SGD(self.parameters(), lr=lr, weight_decay=1e-3)
...
class EEGClassifier(pl.LightningModule):
    ...
    def configure_optimizers(self): ## EEGClassifier
        lr = self.learning_rate
        # if self.train_cond_stage_only:
        #     print(f"{self.__class__.__name__}: Only optimizing conditioner params!")
        #     cond_parms = [p for n, p in self.named_parameters() 
