...
class cond_stage_model(nn.Module):
    def __init__(self, metafile, num_voxels=440, cond_dim=1280, global_pool=True, clip_tune = True, cls_tune = False):
        ...
        if metafile is not None:
            ...
            model.load_checkpoint(metafile['cond_stage_model'])
            # model.load_checkpoint(torch.load('./pretrains/eeg_pretrain/state_dict', map_location='cpu'))
...
# class cond_stage_model(pl.LightningModule):
    def __init__(self, metafile, num_voxels=440, cond_dim=1280, global_pool=True, clip_tune = True, cls_tune = False):
        super().__init__()
        # prepare pretrained fmri mae 
        if metafile is not None:
...
class eLDM:
    ...
    def __init__(self, metafile, num_voxels, device=torch.device('cpu'),
        ...
        self.ckp_path = os.path.join(pretrain_root, 'models/v2-1_768-ema-pruned-mod.ckpt') ### stable diffusion version 2.1
        self.config_path = os.path.join(pretrain_root, 'models/v2-1_768-ema-pruned.yaml')
        ...
        model = instantiate_from_config(config.model) # done
        print('initial model', model)
        pl_sd = torch.load(self.ckp_path, map_location="cpu")['state_dict']
        print('loaded')
       
        ### refinement for model loading checkpoint ###
        m, u = model.load_state_dict(pl_sd, strict=False)
        # full_ckpt = torch.load('pretrains/generation/checkpoint.pth', map_location='cpu')
        # m, u = model.load_state_dict(full_ckpt['model_state_dict'], strict=False)
    ...
    @torch.no_grad()
    def generate(self, data, num_samples, ddim_steps, HW=None, limit=None, state=None, output_path = None):
        ...
        with model.ema_scope():
            ...
            for count, item in enumerate(data):
                ...
                c, re_latent = model.get_learned_conditioning(repeat(latent, 'h w -> c h w', c=num_samples))
